(function () {
  ...
  function createHandlers(ctx) {
    ...
    return {
      ...
      openCfModal(memberId) {
        const state = getState();
        state.ui.cfModal = { open: true, memberId };
        render();
      },
      closeCfModal() {
        const state = getState();
        state.ui.cfModal = { open: false, memberId: null };
        render();
      },
      applyCfReference(memberId, value) {
        const state = getState();
        const member = beamState.findMember(state, memberId);
        if (!member) return;
        member.design = member.design || beamState.createDefaultWoodDesign();
        member.design.cf = Number(value) || 1;
        state.ui.cfModal = { open: false, memberId: null };
        render();
      },
      addLoad(memberId) {
