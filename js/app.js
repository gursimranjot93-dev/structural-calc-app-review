...
(function () {
  ...
  async function boot() {
    if (window.StructuralCalcLibrariesReady && typeof window.StructuralCalcLibrariesReady.then === 'function') {
      await window.StructuralCalcLibrariesReady;
    }
  ...
  boot().catch((error) => {
    ...
    alert('The app could not start. Check the console for details.');
  });
})();
