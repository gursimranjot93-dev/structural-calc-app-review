(function () {
  ...
  function cfReferenceRows() {
    const rows = libs().woodCFReference;
    return Array.isArray(rows) ? rows : [];
  }
  ...
  function renderCfModal(state, member, calc) {
    ...
    const rows = cfReferenceRows();
    const family = calc.family || 'unknown';
    return `
      <div class="cf-modal" data-close-cf-modal>
        <div class="cf-modal-card" onclick="event.stopPropagation()">
          ...
          <div class="muted" style="margin:6px 0 10px;">Applicable family: ${escapeHtml(family)}. Use Apply to copy a reference value into CF.</div>
          <div class="sheet-scroll-table">
            <table class="summary-table compact-summary-table cf-reference-table">
              <thead><tr><th>Label</th><th>Value</th><th>Families</th><th>Notes</th><th>Action</th></tr></thead>
              <tbody>
                ${rows.length ? rows.map((row) => {
                  const families = Array.isArray(row.families) ? row.families : [];
                  const applicable = families.includes('all') || families.includes(family);
                  return `<tr>
                    <td>${escapeHtml(row.label || '')}</td>
                    <td class="num">${formatNum(Number(row.value) || 0, 3)}</td>
                    <td>${escapeHtml(families.join(', '))}</td>
                    <td>${escapeHtml(row.notes || '')}</td>
                    <td><button class="toolbar-btn" data-apply-cf="${escapeAttr(String(row.value))}">${applicable ? 'Apply' : 'Apply'}</button></td>
                  </tr>`;
                }).join('') : '<tr><td colspan="5" class="muted">No CF reference rows loaded.</td></tr>'}
              </tbody>
            </table>
          </div>
        </div>
  ...
  function bindBeamDesignSheetEvents(state, member, handlers) {
    ...
    bindAll('[data-apply-cf]', (el) => el.addEventListener('click', () => handlers.applyCfReference(member.id, el.dataset.applyCf)));
  }
